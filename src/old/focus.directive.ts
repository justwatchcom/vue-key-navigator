...
import { Payload } from './nodeTree'
import { VNode, PluginObject } from 'vue'
import { Vue as _Vue } from 'vue/types/vue'
import { generateAlphabetString } from '../helpers/math-helpers'
...
export class FocusElement implements Payload {
  ...
  constructor (vnode: VNode, params?: { order?: number; isDefault?: () => boolean }) {
    if (!vnode || !vnode.elm) {
      throw new Error('vnode or vnode.elm not defined')
    }
    ...
    if (!element.id) {
      element.id = `focus-el-${generateAlphabetString()}`
    }
  ...
  // TODO Probably should be called `update classes`? This just updates dom, doesn't clean a thing.
  clean (): void {
...
export const FocusDirectivePlugin: PluginObject<NavigationServiceOptions> = {
  install: (Vue: typeof _Vue, options: NavigationServiceOptions = {}): void => {
    // TODO Probably not the best way to keep service.
    // @ts-ignore
    window.navigationService = navigationService = new NavigationService(options)

    if (options.disabled) {
      // Directive will do nothing
      Vue.directive('focus', {})
      return
    }
